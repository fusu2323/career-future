...
<objective>
Implement the resume parsing endpoint logic: file extraction, LLM one-shot extraction with 20s timeout (per D-03), self-correction with partial fallback (per D-05), and integration tests for all STU requirements.
</objective>
...
<introduction>
This plan completes the /resume/parse endpoint. It depends on Plan 06-01 (Wave 1) which created the router skeleton, Pydantic models, and test fixtures.
